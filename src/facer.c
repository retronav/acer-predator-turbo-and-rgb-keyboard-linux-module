...
static const struct key_entry acer_wmi_keymap[] __initconst = {
    ...
    {KE_IGNORE,
     0x84,
     {KEY_KBDILLUMTOGGLE}}, /* Automatic Keyboard background light toggle */
...
/*
 * Gaming functions user-space communication
 * A character drive will be exposed in /dev/acer-gkbbl-static as char block for
 * keyboard static backlight config Config length is 16 bytes
 */
...
MODULE_PARM_DESC(cycle_gaming_thermal_profile,
                 "Set thermal mode key in cycle mode. Disabling it sets the "
                 "mode key in turbo toggle mode");
MODULE_PARM_DESC(
    predator_v4,
 ...
 * GAMING_KBBL_STATIC_MINOR -> used to configure gaming rgb keyboard
backlights by static color from user-space
 */
...
static dev_t
    gkbbl_static_dev; // Global variable to store the static kbbl device
...
static void __init set_quirks(void)
{
  ...
  if (quirks->turbo)
    interface->capability |=
        ACER_CAP_TURBO_OC | ACER_CAP_TURBO_LED | ACER_CAP_TURBO_FAN;

  if (quirks->predator_v4)
    interface->capability |=
        ACER_CAP_PLATFORM_PROFILE | ACER_CAP_FAN_SPEED_READ;
}
...
static bool has_cap(u32 cap) { return interface->capability & cap; }
...
static acpi_status __init AMW0_set_capabilities(void)
{
  ...
  {
    if ((quirks != &quirk_unknown) || !AMW0_set_cap_acpi_check_device())
...
static struct wmi_interface wmid_gaming_interface = {.type = ACER_WMID_GAMING};

static acpi_status WMI_gaming_execute_u8_array(u32 method_id, u8 array[],
                                               size_t array_size, u32 *out)
{
...
static acpi_status WMID_gaming_set_u8_array(u8 array[], size_t array_size,
                                            u32 cap)
{
  u32 method_id = 0;
...
static acpi_status wmid3_get_device_status(u32 *value, u16 device)
{
  ...
  struct acpi_buffer input = {sizeof(struct wmid3_gds_get_input_param),
                              &params};
...
static acpi_status wmid3_set_device_status(u32 value, u16 device)
{
  ...
  struct acpi_buffer get_input = {sizeof(struct wmid3_gds_get_input_param),
                                  &get_params};
  ...
  struct acpi_buffer set_input = {sizeof(struct wmid3_gds_set_input_param),
                                  &set_params};
  struct acpi_buffer output = {ACPI_ALLOCATE_BUFFER, NULL};
...
static void __init type_aa_dmi_decode(const struct dmi_header *header,
                                      void *d)
{
...
static ssize_t gkbbl_drv_write(struct file *file, const char __user *buf,
...
static const struct file_operations gkbbl_dev_fops = {.owner = THIS_MODULE,
...
static int __init gaming_kbbl_cdev_init(void)
{
  ...
  device_create(gkbbl_dev_class, NULL, MKDEV(dev_major, GAMING_KBBL_MINOR),
                NULL, "%s-%d", GAMING_KBBL_CHR, GAMING_KBBL_MINOR);
...
static ssize_t gkbbl_static_drv_write(struct file *file, const char __user *buf,
                                      size_t count, loff_t *offset)
{
  ...
  set_input = (struct acpi_buffer){sizeof(set_params), &set_params};
  ...
  wmi_evaluate_method(WMID_GUID4, 0, ACER_WMID_SET_GAMING_STATIC_LED_METHODID,
                      &set_input, NULL);
  return count;
}

static const struct file_operations gkbbl_static_dev_fops = {
    .owner = THIS_MODULE, .write = gkbbl_static_drv_write};

static struct class *gkbbl_static_dev_class;
static struct gkbbl_device_data gkbbl_static_dev_data;

static int gkbbl_static_dev_uevent(const struct device *dev,
                                   struct kobj_uevent_env *env)
{
  add_uevent_var(env, "DEVMODE=%#o", 0666);
  return 0;
...
static int __init gaming_kbbl_static_cdev_init(void)
{
  ...
  if (err < 0)
  {
    pr_err("Char drive registering for gaming keyboard static backlight "
           "failed: %d\n",
           err);
    return err;
  }
  ...
  device_create(gkbbl_static_dev_class, NULL, gkbbl_static_dev, NULL, "%s-%d",
                GAMING_KBBL_STATIC_CHR, GAMING_KBBL_STATIC_MINOR);
...
static int __init gaming_kbbl_poll_and_enable_zones(void)
{
  ...
  WMI_gaming_execute_u64(ACER_WMID_GET_GAMING_SYS_INFO_METHODID, 0,
                         &gaming_sysinfo);
  /* Turn on all 4 zones */
  WMI_gaming_execute_u64(ACER_WMID_SET_GAMING_LED_METHODID, 8L | (15UL << 40),
                         NULL);
  return 0;
}
...
static int acer_thermal_profile_change(void)
{
  ...
  if (quirks->predator_v4)
  {
    ...
    err = ec_read(ACER_PREDATOR_V4_THERMAL_PROFILE_EC_OFFSET, &current_tp);
...
static int acer_rfkill_init(struct device *dev)
{
  ...
  if (has_cap(ACER_CAP_BLUETOOTH))
  {
    bluetooth_rfkill = acer_rfkill_register(
        dev, RFKILL_TYPE_BLUETOOTH, "acer-bluetooth", ACER_CAP_BLUETOOTH);
  ...
  {
    threeg_rfkill = acer_rfkill_register(dev, RFKILL_TYPE_WWAN, "acer-threeg",
                                         ACER_CAP_THREEG);
  ...
  case WMID_HOTKEY_EVENT:
    ...
    else
    {
      ...
      case KEY_TOUCHPAD_TOGGLE:
        scancode = (device_state & ACER_WMID3_GDS_TOUCHPAD) ? KEY_TOUCHPAD_ON
                                                            : KEY_TOUCHPAD_OFF;
      }
...
static acpi_status __init wmid3_set_function_mode(
    struct func_input_params *params, struct func_return_value *return_value)
{
  acpi_status status;
  union acpi_object *obj;
...
static int __init acer_wmi_input_setup(void)
{
  ...
  status =
      wmi_install_notify_handler(ACERWMID_EVENT_GUID, acer_wmi_notify, NULL);
  if (ACPI_FAILURE(status))
  {
    err = -EIO;
    goto err_free_dev;
...
static int acer_wmi_hwmon_init(void)
{
  ...
  hwmon = devm_hwmon_device_register_with_info(
      dev, "acer", &acer_platform_driver, &acer_wmi_hwmon_chip_info, NULL);
...
static int __init acer_wmi_init(void)
{
   ...
   */
  if (wmi_has_guid(AMW0_GUID1) && !dmi_check_system(amw0_whitelist) &&
      quirks == &quirk_unknown)
  {
